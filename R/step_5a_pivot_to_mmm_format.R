...
#'
#' We pivot our table from Step 4 (which was on a campaign + channel format) to a weekly format. This function requires no parameters.
#'
...
step_5a_pivot_to_mmm_format <- function(
)
{
  ...
  for (j in 1:length(channels_impressions)) {
    cols_to_summarize_imps <-   names(.GlobalEnv$df_ads_step4[grepl(paste0("impressions_", quo_name(channels_impressions[j]), "_after_running_week_"), names(.GlobalEnv$df_ads_step4))])
    df_no_channel_imps <- .GlobalEnv$df_ads_step4 %>%
  ...
  for (j in 1:length(channels_clicks)) {
    cols_to_summarize_clicks <-   names(.GlobalEnv$df_ads_step4[grepl(paste0("clicks_", quo_name(channels_clicks[j]), "_after_running_week_"), names(.GlobalEnv$df_ads_step4))])
    df_no_channel_clicks <- .GlobalEnv$df_ads_step4 %>%
  ...
  for (j in 1:length(channels)) {
    cols_to_summarize_spend <-   names(.GlobalEnv$df_ads_step4[grepl(paste0("spend_", quo_name(channels[j]), "_after_running_week_"), names(.GlobalEnv$df_ads_step4))])
    df_no_channel <- .GlobalEnv$df_ads_step4 %>%
  ...
  for (j in 1:length(channels)) {
    cols_to_summarize_cvr <-   names(.GlobalEnv$df_ads_step4[grepl(paste0("noisy_cvr_", quo_name(channels[j]), "_after_running_week_"), names(.GlobalEnv$df_ads_step4))])
    df_no_channel_cvr <- .GlobalEnv$df_ads_step4 %>%
  ...
  .GlobalEnv$df_ads_step5a_before_mmm <- output
