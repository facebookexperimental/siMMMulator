...
#' Step 5a: Pivot from Our campaign + channel format to a daily format for MMM format
#'
#' We pivot our table from Step 4 (which was on a campaign + channel format) to a daily format.(We will create a weekly format later.)
#'
...
)
{
  ...
  # set up rows to get an intermediate data frame, will do daily first, we will aggregate up to weekly later
  ...
  # for impressions, pivot into a daily format
  if (length(channels_impressions) == 0) { # skip this step if no channels that use clicks were provided
    ...
    for (j in 1:length(channels_impressions)) {
      cols_to_summarize_imps <-   names(df_ads_step4[grepl(paste0("impressions_", quo_name(channels_impressions[j]), "_after_running_day_"), names(df_ads_step4))])
      ...
      #### shift values down the appropriate number of days from when data start that campaign starts
      #### add empty rows to data set so that we can shift downwards
      df_no_channel_imps[nrow(df_no_channel_imps)+1:(n_days-frequency_of_campaigns),] <- NA
      ##### shift
      ...
      df_no_channel_imps <- df_no_channel_imps %>%
        replace(is.na(.), 0) %>%
        mutate(!!paste0("sum_n_", quo_name(channels_impressions[j]), "_this_day") := rowSums(across(where(is.numeric))))
      # input in our results from earlier
      df_ads_step5a_before_mmm[,paste0("sum_n_", quo_name(channels_impressions[j]), "_imps_this_day")] <- df_no_channel_imps[,paste0("sum_n_", quo_name(channels_impressions[j]), "_this_day")]
    }
  }

  # for clicks, pivot into a daily format
  if (length(channels_clicks) == 0) { # skip this step if no channels that use clicks were provided

  } else {
    for (j in 1:length(channels_clicks)) {
      cols_to_summarize_clicks <-   names(df_ads_step4[grepl(paste0("clicks_", quo_name(channels_clicks[j]), "_after_running_day_"), names(df_ads_step4))])
      ...
      #### shift values down the appropriate number of days from when data start that campaign starts
      #### add empty rows to data set so that we can shift downwards
      df_no_channel_clicks[nrow(df_no_channel_clicks)+1:(n_days-frequency_of_campaigns),] <- NA
      ##### shift
      ...
      df_no_channel_clicks <- df_no_channel_clicks %>%
        replace(is.na(.), 0) %>%
        mutate(!!paste0("sum_n_", quo_name(channels_clicks[j]), "_this_day") := rowSums(across(where(is.numeric))))
      # input in our results from earlier
      df_ads_step5a_before_mmm[,paste0("sum_n_", quo_name(channels_clicks[j]), "_clicks_this_day")] <- df_no_channel_clicks[,paste0("sum_n_", quo_name(channels_clicks[j]), "_this_day")]
    }
  }

  # for spend, pivot into a daily format
  for (j in 1:length(channels)) {
    cols_to_summarize_spend <-   names(df_ads_step4[grepl(paste0("spend_", quo_name(channels[j]), "_after_running_day_"), names(df_ads_step4))])
    ...
    #### shift values down the appropriate number of days from when data start that campaign starts
    #### add empty rows to data set so that we can shift downwards
    df_no_channel[nrow(df_no_channel)+1:(n_days-frequency_of_campaigns),] <- NA
    ##### shift
    ...
    df_no_channel <- df_no_channel %>%
      replace(is.na(.), 0) %>%
      mutate(!!paste0("sum_n_", quo_name(channels[j]), "_this_day") := rowSums(across(where(is.numeric))))
    # input in our results from earlier
    df_ads_step5a_before_mmm[,paste0("sum_spend_", quo_name(channels[j]), "_this_day")] <- df_no_channel[,paste0("sum_n_", quo_name(channels[j]), "_this_day")]
  }

  # add noisy CVR to the dataframe
  for (j in 1:length(channels)) {
    cols_to_summarize_cvr <-   names(df_ads_step4[grepl(paste0("noisy_cvr_", quo_name(channels[j]), "_after_running_day_"), names(df_ads_step4))])
    ...
    #### shift values down the appropriate number of days from when data start that campaign starts
    #### add empty rows to data set so that we can shift downwards
    df_no_channel_cvr[nrow(df_no_channel_cvr)+1:(n_days-frequency_of_campaigns),] <- NA
    ##### shift
    ...
    df_no_channel_cvr <- df_no_channel_cvr %>%
      replace(is.na(.), 0) %>%
      mutate(!!paste0("cvr_", quo_name(channels[j]), "_this_day") := rowSums(across(where(is.numeric))))
    # input in our results from earlier
    df_ads_step5a_before_mmm[,paste0("cvr_", quo_name(channels[j]), "_this_day")] <- df_no_channel_cvr[,paste0("cvr_", quo_name(channels[j]), "_this_day")]
  }
