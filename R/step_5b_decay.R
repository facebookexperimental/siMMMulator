...
)
{
  ...
  if (length(true_lambda_decay)!= (length(c(channels_impressions, channels_clicks))) | (any(is.na(true_lambda_decay)) == TRUE)) stop("Did not input in enough numbers for true_lambda_decay. Must have a value specified for each channel.") # error if not enough true_lambda_decay are supplied
  ...
  # apply decay
  intermediate_df <- .GlobalEnv$df_ads_step5a_before_mmm
  ...
  .GlobalEnv$df_ads_step5b <- output
